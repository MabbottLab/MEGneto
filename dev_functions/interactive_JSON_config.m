...
contact_epoching_prompt = {'Enter contact email (config.contact)', ...
'Enter epoching period for resting state data (config.epoching.period)',... 
'Enter threshold for headmotion (config.epoching.headmotion)'};
...
cleaningoptions_prompt = {'Enter 0 or 1 for artifact detection',...
...
'Enter the filter type the muscle artifact'...
'Enter yes or no to indicate if a hilbert transform should be done'...
'Enter a window length for the boxcar/moving average filter'...
'Enter the cut off frequency for muscle artifact cleaning'...
'Enter length of time for trial padding'};
...
cleaningoptions2_prompt = {'Enter length of time for filter padding'...
'Enter length of time for artifact padding'...
'Enter cutoff frequency for jump artifact detection'...
'Enter 0 or 1 to indicate if ICA should be done'...
'Enter 0 or 1 to indicate if noisy trials should be removed'...
'Enter 0 or 1 to indicate if bad trials should be removed'};
...
filteringParams_prompt = {'Specify MEG channels',...
 'Enter yes or no to indicate if a notch filter for line noise should be applied'...
 'Enter frequency of the line noise [x, y]',...
 'Enter yes or no to indicate if a bandpass filter should be applied'...
 'Enter bandpass frquency [x, y]', 'Enter filter order', ...
 'Enter sampling rate of the data', 'Enter MEG model'};
...
%% config.taskFunc
taskFunc_prompt = {'Enter function to parse data into trials',...
...
%% config.task
task_prompt = {'Enter 1 or 0 to specify if there is rest data', ...
'Enter name of marker to epoch around',...
'Enter name of marker to include once (leave as blank)',...
'Enter name of marker to exclude (leave as blank)',...
'Enter marker to gain reaction time information on', ...
'Enter  true or false to specify whether you only want information on the number of trials'...
'Specify delay time to correct for', 'Enter epoching period',...
'Specify markers that designate a correct trials',...
...
'Specify template MNI grid resolution in mm',...
'Specify yes or no for tight grid',...
...
'Specify atlas filepath',...
'Specify the input coordinate',...
'Specify plotting method of MRI volumes',...
'Specify dimension to slice on if method = slice'};
...
beamforming2_prompt = {'Specify the number of slices in method = slice',...
...
'Enter vartrllength',...
'Enter yes or no to specify is data should be projected to dominant eigenvector'};
...
%% config.connectivity
connectivity_prompt = {'Enter metric for connectivity analysis',...
'Specify frequency bands',...
'Specify method of generating a representative timeseries for each ROI'};
