...
all_conn_mat = nan(num_sources, num_sources, ... % num_nodes x num_nodes x ...
    ...
    length(subj_match.ds));                 % num_subjects
...
for fq = 1:length(config.connectivity.filt_freqs)
    for ss = 1:12
        ...
        % COMPUTE 1000 PERMUTATIONS TO MAKE
        ...
        for ii = 1:1000
            ...
            temp_content    = temp(indices);
            perms(ii, :)       = temp_content';  % match number of control trials to interest trials
        end
            C               = unique(perms, 'rows');
            content         = C(1:500, :);
        parfor jj = 1:500
            ...
            all_conn_mat(:,:,jj,ss) = conn_mat;
        end
    end
    
    %%% SAVE MASTER CONNECTIVITY MATRIX WITH ALL PARTICIPANTS
    save([paths.anout_grp '/fcp_5_' band_names{fq} '_permutations_mats_' config.connectivity.method '.mat'],'all_conn_mat','-v7.3');
end
