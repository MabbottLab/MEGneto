function fcp_2_PreprocessingICA(paths, calculation, interactive)
...
if calculation == true% && false
    for ss = rangeOFsubj %1:rangeOFsubj
        ...
        % manuals removal of bad channels
        if config.cleaningOptions.rmBadChannels == 2
            channel_out = [ssSubjPath(ss) '/' fcp2_output.subj_dir_rawchanfig];
            saveSensorsToFile(paths, subj_match, ss, channel_out); % view sensor data
        end
    end
end
if interactive == true% || true
    for ss = rangeOFsubj
        if config.cleaningOptions.rmBadChannels == 2
            channel_out = [ssSubjPath(ss) '/' fcp2_output.subj_dir_rawchanfig];
            disp(['Please check the images in ' channel_out ' and identify bad channels.'])
            [goodChannels,badChannels]=inputBADchannels(rawdata,ss,subj_match);
            fcp2_output.bad_chann{ss} = badChannels ;
        else
            fcp2_output.bad_chann{ss} = {};
        end
        writecell(fcp2_output.bad_chann{ss},[paths.(subj_match.pid{ss}) '/badchannels.csv']);
    end
end
if calculation == true
    ...
    for ss = rangeOFsubj
        ...
        if config.cleaningOptions.rmBadChannels == 1
            ...
            cfg.badchannel     = fcp2_output.bad_chann{ss};
        ...
        elseif config.cleaningOptions.artifact.icaClean == 1
            ...
            if  ~isempty(fcp2_output.bad_chann{ss})
                ...
                %ones are not unique (no idepended components)
                rmCh = strcat('-',fcp2_output.bad_chann{ss});
                selchan = ft_channelselection({'all' rmCh{1,:}}, data_noisecorr.label);
                data_resampICArmBadCh = ft_selectdata(data_resampICA,'channel',selchan);
