function freeviewing_fcp_4_beamforming(paths)
...
%
% OUTPUTS:
%   .mat (part 1 and part 2 beamforming for each ppt)
...
%   atlas = ft_read_atlas([megneto_path '/external/atlas/mmp.mat']); % path to the atlas
...
%%% ADD FIDUCIALS
    if matches(subj_match.pid{ss}(5:end), 'P01')
        fid_info = load([OIRM_dataset_MRI '/08_MRI/08.mat']);
    elseif matches(subj_match.pid{ss}(5:end), 'P02')
        fid_info = load([OIRM_dataset_MRI '/09_MRI/09.mat']);
    else
        fid_info = load([OIRM_dataset_MRI subj_match.pid{ss}(5:end) '_MRI/' subj_match.pid{ss}(5:end) '.mat']);
    end
...
%%% VECTOR - Time Domain Source Reconstruction ----------------------------
    ...
    %%% Split data into 2 parts to facilitate beamforming at a finer resolution
    for split = 1:2
        cfg = [];
        cfg.dataset = tlock;
        ...
        if contains(config.beamforming.atlas.filepath, 'mmp') % if MMP glasser atlas
            megneto_path        = fileparts(which('freeviewing_fcp_4_beamforming.m'));
        ...
        %all_source_timeseries  = cell(length(projection.trial), ... % set up empty matrix to store variance explained of first principial component for each trial and region of interest
        %                             length(max(size(atlas.tissuelabel))));
        for t = 1:projection.df
            ...
            for i = 1:max(size(atlas.tissuelabel))
                ...
                %all_source_timeseries{t,i}  = source_timeseries;
                %             if i == 126 || i == 306 || i == 127 || i == 307 || i ==155 || i ==335 || i ==153 || i==333 || i==154 || i==334 || i==160 || i==340
                %                 all_source_timeseries{t,i}  = source_timeseries;
                %             end
        ...
        if split == 1
            if isnan(var_explained) % if the variance explained has not been populated, don't save it
                save([ssSubjPath(ss) '/atlas_beamforming_results_part1.mat'],'catmatrix', 'srate','coords','-mat','-v7.3')
                %save([ssSubjPath(ss) '/all_source_timeseries.mat'], 'all_source_timeseries', 'srate','coords','-mat','-v7.3')
            else
                save([ssSubjPath(ss) '/atlas_beamforming_results_part1.mat'],'catmatrix', 'var_explained', 'srate','coords','-mat','-v7.3')
            end
        elseif split == 2
            if isnan(var_explained) % if the variance explained has not been populated, don't save it
                save([ssSubjPath(ss) '/atlas_beamforming_results_part2.mat'],'catmatrix', 'srate','coords','-mat','-v7.3')
            else
                save([ssSubjPath(ss) '/atlas_beamforming_results_part2.mat'],'catmatrix', 'var_explained', 'srate','coords','-mat','-v7.3')
            end
        end
