...
for this_seed = seed_regions
    ...
    % MaxT procedure
    [p_val, obs_diffs, eff_size] = permutationTest(seed_mat(:,logical(design_matrix(:,2))), ...
                        seed_mat(:,logical(design_matrix(:,1))), ...
                        num_bootstraps, 'plotresult', 0, 'showprogress', 0);
                    
%     [pval, corr_obs, crit_corr, est_alpha, seed_state] = ...
%     mult_comp_perm_t2(seed_mat(:,logical(design_matrix(:,2)))',seed_mat(:,logical(design_matrix(:,1)))',...
%                         10000,0,0.05,0,'td');
