...
if exist(paths.paths, 'file') && ~overwrite % then retrieve it and return
    ...
    for ii = 1:length(all_participants)
        ...
        if (length(strfind(all_participants_subset, '_')) > 1) || (length(strfind(all_participants_subset, '.')) > 1) 
            ...
            if contains(all_participants_subset{ii}, '_') % check whether the character array contains an underscore 
                extracted_pid = extractBetween(all_participants_subset{ii}, startpos+1, '_'); % extract characters between the last '/' and '_'
            else
                extracted_pid = extractBetween(all_participants_subset{ii}, startpos+1, '.'); % extract characters between the last '/' and '.'
            end 
        end
